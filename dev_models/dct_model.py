...
class DCTModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.input_seg = self.Tensor(
            nb, opt.input_nc, self.inputSize, self.inputSize)
        self.input_heatmap = self.Tensor(
            nb, opt.input_nc, self.inputSize, self.inputSize)
        ...
        if opt.dist:
            self.encoder = DistributedDataParallel(
                self.encoder, device_ids=[torch.cuda.current_device()],find_unused_parameters=True)
        if self.isTrain:
    ...
    def set_input(self, input):
        ...
        input_seg = input['seg_img']
        self.input_seg.resize_(input_seg.size()).copy_(input_seg)
        input_heatmap = input['heatmap_img']
        self.input_heatmap.resize_(input_heatmap.size()).copy_(input_seg)
    ...
    def forward(self):
        ...
        if not self.aux_as_main:
            self.output,self.seg_output,self.featmap_output = self.encoder(self.input_img, self.input_IUV,self.input_seg,self.input_heatmap)
        else:
            self.output,self.seg_output,self.featmap_output = self.encoder(self.input_IUV, self.input_IUV,self.input_seg,self.input_heatmap)
        ...
        # get parsing params for loss prepare.........
        self.seg_final_params = self.seg_output + self.mean_params
        self.seg_cam_params = self.seg_final_params[:, :cam_dim]
        self.seg_pose_params = self.seg_final_params[:, cam_dim: (
            cam_dim + pose_dim)]
        self.seg_shape_params = self.seg_final_params[:, (cam_dim + pose_dim):]
        self.seg_global_rotation = self.seg_pose_params[:, :3].clone()
        if not self.opt.isTrain:
            self.seg_pose_params[:, :3] = 0.0
        self.seg_smpl_params = torch.cat(
            [self.seg_pose_params, self.seg_shape_params], dim=1)
        self.seg_verts_3d, self.seg_kp_3d, _ = self.smpl(
            self.seg_shape_params, self.seg_pose_params)
        if not self.opt.isTrain:
            # for visualization and 2D prediction, use the predicted global rotation
            self.segvis_pose_params = self.seg_pose_params.clone()
            self.segvis_pose_params[:, :3] = self.seg_global_rotation.clone()
            self.seg_verts_vis, self.segvis_kp_3d, _ = self.smpl(
                self.seg_shape_params, self.segvis_pose_params)
            self.seg_verts_2d = batch_orth_proj_idrot(
                self.seg_verts_vis, self.seg_cam_params)
            self.seg_kp_2d = batch_orth_proj_idrot(
                self.segvis_kp_3d, self.seg_cam_params)
        else:
            self.seg_verts_2d = batch_orth_proj_idrot(
                self.seg_verts_3d, self.seg_cam_params)
            self.seg_kp_2d = batch_orth_proj_idrot(
                self.seg_kp_3d, self.seg_cam_params)

        # get featmap params for loss prepare......
        self.heatmap_final_params = self.featmap_output + self.mean_params
        self.heatmap_cam_params = self.heatmap_final_params[:, :cam_dim]
        self.heatmap_pose_params = self.heatmap_final_params[:, cam_dim: (
            cam_dim + pose_dim)]
        self.heatmap_shape_params = self.heatmap_final_params[:, (cam_dim + pose_dim):]
        self.heatmap_global_rotation = self.heatmap_pose_params[:, :3].clone()
        if not self.opt.isTrain:
            self.heatmap_pose_params[:, :3] = 0.0
        self.heatmap_smpl_params = torch.cat(
            [self.heatmap_pose_params, self.heatmap_shape_params], dim=1)
        self.heatmap_verts_3d, self.heatmap_kp_3d, _ = self.smpl(
            self.heatmap_shape_params, self.heatmap_pose_params)
        if not self.opt.isTrain:
            # for visualization and 2D prediction, use the predicted global rotation
            self.heatmapvis_pose_params = self.heatmap_pose_params.clone()
            self.heatmapvis_pose_params[:, :3] = self.heatmap_global_rotation.clone()
            self.heatmap_verts_vis, self.heatmapvis_kp_3d, _ = self.smpl(
                self.heatmap_shape_params, self.heatmapvis_pose_params)
            self.heatmap_verts_2d = batch_orth_proj_idrot(
                self.heatmap_verts_vis, self.heatmap_cam_params)
            self.heatmap_kp_2d = batch_orth_proj_idrot(
                self.heatmapvis_kp_3d, self.heatmap_cam_params)
        else:
            self.heatmap_verts_2d = batch_orth_proj_idrot(
                self.heatmap_verts_3d, self.heatmap_cam_params)
            self.heatmap_kp_2d = batch_orth_proj_idrot(
                self.heatmap_kp_3d, self.heatmap_cam_params)
    ...
    def backward_E(self):
        ...
        # add heatmap loss
        self.heatmapkp_2d_loss = self.loss_util._keypoint_2d_loss(
            self.keypoints, self.heatmap_kp_2d, self.keypoints_weights)
        self.heatmapkp_2d_loss *= self.opt.kp_loss_weight
        self.loss = (self.loss + self.heatmapkp_2d_loss*0.5)

        if self.has_smpl_param_loss:
            self.heatmap_smpl_params_loss = self.loss_util._smpl_params_loss(
                self.smpl_params, self.heatmap_smpl_params, self.smpl_params_weight, self.smpl_loss_weight)
            self.heatmap_smpl_params_loss *= self.opt.loss_3d_weight
            self.loss = (self.loss + self.heatmap_smpl_params_loss)

        # add parsing loss
        # self.segkp_2d_loss = self.loss_util._keypoint_2d_loss(
        #     self.keypoints, self.seg_kp_2d, self.keypoints_weights)
        # self.segkp_2d_loss *= self.opt.kp_loss_weight
        # self.loss = (self.loss + self.segkp_2d_loss*0.5)

        if self.has_smpl_param_loss:
            self.seg_params_loss = self.loss_util._smpl_params_loss(
                self.smpl_params, self.seg_smpl_params, self.smpl_params_weight, self.smpl_loss_weight)
            self.seg_params_loss *= self.opt.loss_3d_weight
            self.loss = self.loss + self.seg_params_loss


        # add shape loss
        self.criterion_shape = nn.L1Loss()
        self.pose_params_dim = self.opt.pose_params_dim
        shape_params2 = self.smpl_params[:, self.pose_params_dim:]
        self.loss = (self.loss + self.criterion_shape(self.pred_shape_params, shape_params2)*0.5)
    ...
    def compute_loss(self):
        ...
        # self.pose_params_dim = self.opt.pose_params_dim
        # shape_params2 = self.smpl_params[:, self.pose_params_dim:]
        # shape_loss = self.criterion_shape(self.pred_shape_params, shape_params2)*0.5
        # res_dict['shape_loss'] = shape_loss.detach().cpu().numpy()
        ...
        if self.has_smpl_param_loss:
            self.smpl_params_loss = self.loss_util._smpl_params_loss(
                self.smpl_params, self.seg_smpl_params, self.smpl_params_weight, self.smpl_loss_weight)
            res_dict['smpl2_loss'] = self.smpl_params_loss.detach().cpu().numpy()
        return res_dict
    ...
    def get_pred_result(self):
        pred_result = OrderedDict({
            ...
            'pred_kp_3d': self.pred_kp_3d.cpu().numpy(),
            'pred_kp_2d': self.pred_kp_2d.cpu().numpy(),
