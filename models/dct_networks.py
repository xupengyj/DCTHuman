...
class DCTEncoder(nn.Module):
    ...
    def forward(self, main_input, aux_input,seg_input,heatmap):
        main_feat,main_pose_feat = self.main_encoder(main_input)
        seg_feat,seg_pose_feat = self.main_encoder(seg_input)
        heatmap_feat,heatmap_pose_feat = self.main_encoder(heatmap)
        
        if self.two_branch:
            aux_feat,aux_pose_feat = self.aux_encoder(aux_input)
